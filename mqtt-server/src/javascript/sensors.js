...
function webSocketConnect() {
  ...
  webSocket.onmessage = function(msg) {
    ...
    if (sensorId) { // Doesn't allow for default/blank sensors
...
function createSensorDiv(messageData) {
  let convertedDate = new Date(messageData.updated).toLocaleString();
  let div = "";
  div += `
...
function setDivColor(messageData) {
  ...
  let tempTooHot = ( temp > TEMP_HIGH );
  let tempTooCold = ( temp < TEMP_LOW );
